...
public class BlockChain {
    ...
    public void updateMaxHeightNode() {
        ...
        for (BlockNode b : _blockchain) {
            if (b._height > currentMaxHeightNode._height) { //return the blocks with biggest height
                currentMaxHeightNode = b;
            } else if (b._height == currentMaxHeightNode._height) {
                if (currentMaxHeightNode._createAt.after(b._createAt)) { //If there are multiple blocks at the same height, return the oldest block
                    currentMaxHeightNode = b;
                }
            }
        }
    ...
    public BlockNode getParentNode(byte[] blockHash) {  //return the blocknode
    ...
    public class BlockNode { //create a class to store the block and its height,utxopool, transactionpool as well as its creating time
    ...
    public BlockChain(Block genesisBlock) {
        ...
        for (int i = 0; i < genesisBlock.getCoinbase().numOutputs(); i++) {
            utxoPool.addUTXO(new UTXO(genesisBlock.getCoinbase().getHash(),i),genesisBlock.getCoinbase().getOutput(i)); //HashMap<UTXO, Transaction.Output>
        }
        transPool.addTransaction(genesisBlock.getCoinbase()); // addTransaction(Transaction tx) , H.put(hash, tx)
        for (Transaction t : genesisBlock.getTransactions()) {
            if (t != null) {
                for (int i=0;i<t.numOutputs();i++) {
                    Transaction.Output output = t.getOutput(i);
                    UTXO utxo = new UTXO(t.getHash(),i); //private byte[] txHash  private int index
    ...
    public boolean addBlock(Block block) {
        ...
        //compare height      {@code height > (maxHeight - CUT_OFF_AGE)}
